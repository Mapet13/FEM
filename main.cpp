...
namespace math_utilities {
	...
	auto gaussJordan(std::vector<std::vector<double>> A, std::vector<double> b) {
		...
		for (const auto i : std::views::iota(0, static_cast<int>(A.size())) | 
			...
			for (const auto j : std::views::iota(i + 1u, A.size()))
...
auto getMatrixA(const int elements_count, const auto de, const double width) {
	...
	for (const auto i : std::views::iota(1, elements_count)) {
		for (const auto j : std::views::iota(i - 1, i + 2)) { // abs(i - j) <= 1
			if (j == 0 || j == elements_count) continue;

			A[i][j] = A[j][i] = -math_utilities::integrate(
				[&de, i, j](const auto x) { return de(i)(x) * de(j)(x); },
				(i == j)
				? Range{ (i - 1) * width, (i + 1) * width }
...
auto getMatrixL(const int elements_count, const auto de, const auto e, const double width) {
	...
	for (const auto i : std::views::iota(1, elements_count)) {
		...
		const auto l1 = [&e, i](const auto x) { return 4 * std::numbers::pi * sulution_constants::G * sulution_constants::p(x) * e(i)(x); };
		const auto l2 = [&de, i](const auto x) { return sulution_constants::dshifted(x) * de(i)(x); };
...
std::vector<double> MES(const int elements_count) {
	const auto width = (sulution_constants::solution_range.end - sulution_constants::solution_range.begin) / elements_count;
	const auto [e, de] = get_base_functions(width);
	
	return math_utilities::gaussJordan(
		getMatrixA(elements_count, de, width), 
		getMatrixL(elements_count, de, e, width)
	);
}	
